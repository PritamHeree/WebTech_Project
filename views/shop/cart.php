...
<?php if (empty($_SESSION['cart'])): ?>
    <!-- 
       Mobile-first design decision: Interactive Empty State!
       Instead of a generic text link, we show a beautiful high-contrast empty layout
       with a clear CTA that redirects the user back to the catalog.
    -->
...
<?php else: ?>
    ...
    <div class="cart-layout">
        ...
        <div class="cart-items-list" id="cart-container">
            <?php 
            $grandTotal = 0;
            $menuModel = new MenuItem($this->pdo); // Fetch item details directly to get image_path
            ...
            <div class="cart-item-card" id="cart-row-<?= $id ?>" data-price="<?= $item['price'] ?>">
                ...
                <!-- Main item details and adjust actions -->
                <div class="cart-item-info">
                    ...
                    <div class="cart-item-meta">
                        <!-- Pill-shaped tactile quantity panel -->
                ...
                <!-- Subtotal readout aligned right -->
...
<script>
/**
 * AJAX Quantity controller sending updates to server-side session
 * and updating the visual viewport values without page refreshes.
 */
async function updateQuantity(id, action) {
    const result = await fetchJson('<?php echo url("/api/cart/update"); ?>', { id: id, action: action });
    if (result.success) {
        // Sync global cart counter in page header
        ...
        if (currentQty <= 0) {
            // Smoothly remove item from list
            ...
            setTimeout(() => {
                ...
                if (result.cart_count === 0) {
                    location.reload(); // Refresh to render the premium empty state
                }
            }, 250);
        } else {
...
/**
 * Remove items completely from the user's cart session.
 */
async function removeFromCart(id) {
    ...
    if (result.success) {
        ...
        // Transition item card away smoothly
        const row = document.getElementById('cart-row-' + id);
        row.style.opacity = '0';
        row.style.transform = 'scale(0.9)';
        setTimeout(() => {
            ...
            if (result.cart_count === 0) {
                location.reload(); // Trigger empty state UI
            }
        }, 250);
